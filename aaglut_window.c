...
#ifdef AAPLOT_ICON
#include <X11/xpm.h>
#include "aaplot_icon.xpm"
#endif

/* -- PRIVATE FUNCTIONS ---------------------------------------------------- */

#if TARGET_HOST_WINCE
#include <aygshell.h>                  /*! \todo Move this to aaglut_internal.h */
/*!
                              ...
                              unsigned char layer_type )
{
    ...
    flags = flags | PFD_SUPPORT_OPENGL;
    if( ogState.DisplayMode & GLUT_DOUBLE )
                   ...
                   GLboolean isSubWindow )
{
    ...
    */
    ...
    window->State.IsOffscreen = GL_FALSE;
    if( GLUT_OFFSCREEN & ogState.DisplayMode )
    {
        /*
    ...
    if( !window->State.IsOffscreen )
    {
        ...
        if( window->IsClientMenu || window->IsMenu )
            SetWindowPos(
                window->Window.Handle,
                HWND_TOPMOST,
                0, 0, 0, 0,
...
Pixmap image, shape;
XWMHints win_hints;
XpmCreatePixmapFromData(display, window,aaplot_icon_xpm, &image, &shape, NULL);

/* win_hints = XAllocWMHints();  ->   error: incompatible types in assignment */
    ...
    if( GL_FALSE == ogStructure.Window->State.IsOffscreen )
    {
        ...
        if( !ogStructure.Window->Parent )
            ...
            XUnmapWindow( ogDisplay.Display,
                          ogStructure.Window->Window.Handle );
//aapo  XFlush( ogDisplay.Display ); /*! \todo Shouldn't need this XFlush */
